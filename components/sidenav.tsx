...
import {
  ...
  Building2, ShieldCheck, Target, RefreshCw, PlusCircle,
} from 'lucide-react';
...
const MOBILE_NAV_ITEMS = NAV_ITEMS.filter((item) =>
  ['/dashboard', '/accounts', '/transactions', '/settings'].includes(item.href)
);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-4 shrink-0 pt-4 border-t border-border/50 flex flex-col gap-3 bg-card/90">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-card/95 backdrop-blur-md border-t border-border flex items-center justify-around px-2 py-2 safe-area-inset-bottom">
        ...
        <Link href="/transactions?add=true" onClick={() => setOpen(false)} className="-mt-7 flex flex-col items-center gap-0.5 text-primary" aria-label="Add transaction">
          <span className="grid size-12 place-items-center rounded-full border-4 border-background bg-primary text-primary-foreground shadow-lg"><PlusCircle className="h-6 w-6" /></span>
          <span className="text-[10px] font-semibold">Add</span>
        </Link>
        {MOBILE_NAV_ITEMS.slice(2).map((item) => {
          const Icon = item.icon;
          const isActive = pathname === item.href || (item.href !== '/dashboard' && pathname?.startsWith(item.href));
          return <Link key={item.href} href={item.href} onClick={(event) => handleNavigation(event, item.href)} aria-disabled={isNavigating} className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-muted-foreground hover:text-foreground'}`}><div className={`p-1.5 rounded-lg transition-all ${isActive ? 'bg-blue-100 dark:bg-blue-950' : ''}`}>{isNavigating && navigationTarget === item.href ? <LoaderCircle className="h-5 w-5 animate-spin" /> : <Icon className="h-5 w-5 stroke-[2]" />}</div><span className="text-[10px] font-semibold leading-tight">{item.label}</span></Link>;
        })}
      </nav>
    </>
  );
